class ResultFlash
  ...
  def phrases
    ...
    if @result.winner_previous_ladder_rank == 1 &&
       [2, 3].include?(@result.loser_previous_ladder_rank)
      {
        winner: "#{@result.winner_name} defended the title",
        loser:  "against pretender #{@result.loser_name}",
      }
    elsif @result.loser_previous_ladder_rank == 1 &&
       [2, 3].include?(@result.winner_previous_ladder_rank)
      {
        winner: "#{@result.winner_name} takes the top position",
        loser:  "relegating #{@result.loser_name} to #{loser_new_ordinal}",
      }
    elsif @result.loser_previous_ladder_rank < @result.winner_previous_ladder_rank
      {
